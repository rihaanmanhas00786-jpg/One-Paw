...
const App: React.FC = () => {
  ...
  if (!user) return <Onboarding onComplete={(name) => {
    const newUser = { name, joinedDate: new Date().toLocaleDateString('en-IN') };
    localStorage.setItem('onepaw_user', JSON.stringify(newUser));
    setUser(newUser);
  }} />;
