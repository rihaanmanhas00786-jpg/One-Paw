...
import { 
  ...
  Camera, Lock, UserCircle, Megaphone, Heart, Users, Info, Phone, Landmark, Menu, X, Star, GraduationCap
} from 'lucide-react';
...
const HomePage: React.FC<HomeProps> = ({ onNavigate, onOpenReport, onSOS, onOpenVolunteer }) => {
  ...
  return (
    <div className="w-full min-h-screen bg-[#FFF8F0] pb-40 relative font-sans selection:bg-orange-200">
      ...
      <section className="w-full max-w-7xl mx-auto px-4 md:px-6 pt-10 pb-20 grid grid-cols-1 lg:grid-cols-3 gap-10 items-stretch">
        ...
        <div className="bg-white/60 p-8 md:p-10 rounded-[50px] border border-orange-200 shadow-xl backdrop-blur-sm order-2 lg:order-1 flex flex-col">
          <div className="flex items-center gap-3 mb-6 text-[#4A2C2A]">
              ...
              <h2 className="font-black uppercase tracking-widest text-xs leading-none">The Mission</h2>
          </div>
          <p className="text-[#4A2C2A] text-[13px] md:text-sm leading-relaxed font-medium italic opacity-90 flex-1">
          ...
          <div className="mt-10 pt-8 border-t border-orange-100/50">
             <p className="text-[#4A2C2A] text-[10px] font-black uppercase tracking-[0.2em] mb-4 opacity-50">Academic Initiative:</p>
             <div className="bg-[#4A2C2A] text-[#FFF8F0] p-6 rounded-[32px] shadow-2xl relative overflow-hidden group">
                <div className="absolute -top-2 -left-2 text-white/5 rotate-12"><Landmark size={80}/></div>
        ...
        <div className="text-center flex flex-col items-center justify-center order-1 lg:order-2 py-6">
          ...
          <p className="text-orange-900 font-black text-[11px] md:text-xs mt-6 uppercase tracking-[0.25em] max-w-[360px] leading-relaxed opacity-80 mx-auto text-center">
             An Integrated Animal Welfare, Rescue and Support System
          </p>
        </div>

        {/* RIGHT: Meet the Team (UPGRADED WITH ROLES & MENTORS) */}
          ...
          <div className="flex flex-col gap-6">
             ...
             <div className="space-y-2 border-b border-orange-100 pb-4">
                <div className="flex items-center gap-2 text-[#4A2C2A]">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">Academic Oversight</p>
                </div>
                <div className="grid grid-cols-1 gap-1">
                    <p className="text-[11px] font-bold text-slate-800 leading-tight">Prof. Syeda Afshana <span className="text-[9px] opacity-40 uppercase font-black ml-1">(Coordinator)</span></p>
                    <p className="text-[11px] font-bold text-slate-800 leading-tight">Dr. Baasit Abubakr <span className="text-[9px] opacity-40 uppercase font-black ml-1">(Project Mentor)</span></p>
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 gap-y-3">
                ...
                <TeamMember name="Tarooba" role="Lead UI/UX Design & Creative" />
                <TeamMember name="Fariya" role="Data & Research Specialist" />
                <TeamMember name="Zikra" role="Quality Assurance & Testing" />
                <TeamMember name="Yameen" role="Presentation & Outreach" />
             </div>
          </div>
        </div>
      </section>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 md:px-6 py-16 border-t border-orange-200/30">
          <div className="flex flex-col items-center mb-16 text-center">
            ...
            <div className="h-1.5 w-32 bg-red-600 rounded-full mb-6 shadow-[0_0_15px_rgba(220,38,38,0.3)]"></div>
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.5em] leading-relaxed">Official J&K Animal Husbandry Department Contacts</p>
          </div>

          <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
              <div className="bg-white rounded-[60px] p-8 md:p-12 shadow-2xl border border-orange-100 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 text-emerald-50/50 -z-0"><Landmark size={140}/></div>
                  <h3 className="text-2xl font-black text-emerald-700 uppercase italic mb-10 border-b border-emerald-50 pb-6 flex items-center gap-4 relative z-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                      {contactData.kashmir.map((item, idx) => (
                          <ContactCard key={idx} district={item.district} name={item.name} phone={item.phone} />
                      ))}
                  </div>
              </div>

              <div className="bg-white rounded-[60px] p-8 md:p-12 shadow-2xl border border-orange-100 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 text-blue-50/50 -z-0"><Landmark size={140}/></div>
                  <h3 className="text-2xl font-black text-blue-700 uppercase italic mb-10 border-b border-blue-50 pb-6 flex items-center gap-4 relative z-10">
      ...
      <div className="w-full max-w-4xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-4 mb-32">
          <button onClick={onOpenReport} className="bg-[#C0392B] text-white p-6 md:p-8 rounded-[40px] flex items-center justify-between shadow-2xl hover:bg-red-700 transition-all group active:scale-95">
              <div className="flex items-center gap-5"><div className="p-4 bg-white/10 rounded-2xl group-hover:rotate-12 transition-transform"><Camera size={28}/></div><div className="text-left"><h4 className="font-black italic uppercase text-lg md:text-xl tracking-tighter leading-none">Report Incident</h4><p className="text-[10px] font-bold opacity-60 uppercase mt-1 tracking-widest leading-none">Submit Live GPS Location</p></div></div>
          ...
          <button onClick={onOpenVolunteer} className="bg-emerald-600 text-white p-6 md:p-8 rounded-[40px] flex items-center justify-between shadow-2xl hover:bg-emerald-700 transition-all group active:scale-95">
              <div className="flex items-center gap-5"><div className="p-4 bg-white/10 rounded-2xl group-hover:rotate-12 transition-transform"><Heart size={28} fill="white"/></div><div className="text-left"><h4 className="font-black italic uppercase text-lg md:text-xl tracking-tighter leading-none">Join Network</h4><p className="text-[10px] font-bold opacity-60 uppercase mt-1 tracking-widest leading-none">Register as a Volunteer</p></div></div>
...
// COMPONENT: Individual Team Member
const TeamMember = ({ name, role }: { name: string, role: string }) => (
    <div className="flex items-start gap-2.5 group">
        <div className="w-1.5 h-1.5 rounded-full bg-orange-500 mt-1.5 shadow-sm group-hover:scale-150 transition-transform"></div>
        <div>
...
const NavBtn = ({ label, onClick }: any) => (
  <button onClick={onClick} className="text-[10px] font-black uppercase text-slate-500 hover:text-orange-600 transition-all tracking-widest border-b-2 border-transparent hover:border-orange-200 pb-1">{label}</button>
);
...
const ContactCard = ({ district, name, phone }: any) => (
    <div className="bg-slate-50 p-5 rounded-[28px] border border-slate-100 flex items-center justify-between hover:bg-white hover:shadow-xl hover:border-orange-200 transition-all group active:scale-95">
        <div className="text-left">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none mb-1">{district}</p>
            <h5 className="font-bold text-slate-800 text-[13px] leading-tight">{name}</h5>
        </div>
