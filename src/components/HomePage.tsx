...
import { 
  ...
  Camera, Lock, UserCircle, Megaphone, Heart, Users, Info, Phone, Landmark, Menu, X, GraduationCap, Zap
} from 'lucide-react';
    ...
    <div className="w-full min-h-screen bg-[#FFF8F0] pb-40 relative font-sans">
      ...
      <section className="w-full max-w-7xl mx-auto px-4 md:px-6 pt-10 pb-16 grid grid-cols-1 lg:grid-cols-3 gap-10 items-stretch">
        ...
        <div className="bg-white/60 p-8 md:p-10 rounded-[50px] border border-orange-200 shadow-xl backdrop-blur-sm order-2 lg:order-1 flex flex-col">
          ...
          <div className="mt-8 pt-6 border-t border-orange-100/50 text-center">
                ...
                <p className="text-[11px] font-bold text-[#4A2C2A] italic">Students of <span className="text-orange-600 font-black">Design your Degree</span>,<br/> University Of Kashmir, Group A</p>
          </div>
        </div>
        ...
        <div className="text-center flex flex-col items-center justify-center order-1 lg:order-2 py-6">
          ...
          <h1 className="text-7xl md:text-8xl font-black text-[#4A2C2A] tracking-tighter italic uppercase leading-none text-center">OnePaw</h1>
          <p className="text-orange-900 font-black text-xs md:text-[13px] mt-6 uppercase tracking-[0.2em] max-w-[380px] leading-relaxed opacity-80 mx-auto text-center">An Integrated Animal Welfare, Rescue and Support System</p>
        </div>
          ...
          <div className="flex flex-col gap-6">
             ...
             <div className="space-y-2 border-b border-orange-100 pb-4 text-center lg:text-left">
                <div className="flex items-center justify-center lg:justify-start gap-2 text-[#4A2C2A]">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest">Academic Supervision</p>
                </div>
             ...
             <div className="grid grid-cols-1 gap-y-2.5">
                <TeamMember name="Rehaan" role="Team Lead & Software Development" />
      ...
      <section className="w-full max-w-7xl mx-auto px-4 md:px-6 py-16 border-t border-orange-200/30">
          <div className="flex flex-col items-center mb-16 text-center">
            <h2 className="text-4xl md:text-5xl font-black text-[#4A2C2A] uppercase italic tracking-tighter mb-4 leading-none text-center">Emergency Directory</h2>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
              <div className="bg-white rounded-[60px] p-8 md:p-12 shadow-2xl border border-orange-100 relative overflow-hidden">
                  <h3 className="text-2xl font-black text-emerald-700 uppercase italic mb-10 border-b border-emerald-50 pb-6 flex items-center gap-4 relative z-10 leading-none"><div className="p-3 bg-emerald-100 rounded-2xl"><MapPin size={24}/></div> Kashmir Division</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                      {contactData.kashmir.map((item, idx) => (<ContactCard key={idx} {...item} />))}
                  </div>
              </div>
              <div className="bg-white rounded-[60px] p-8 md:p-12 shadow-2xl border border-orange-100 relative overflow-hidden">
                  <h3 className="text-2xl font-black text-blue-700 uppercase italic mb-10 border-b border-blue-50 pb-6 flex items-center gap-4 relative z-10 leading-none"><div className="p-3 bg-blue-100 rounded-2xl"><MapPin size={24}/></div> Jammu Division</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                      {contactData.jammu.map((item, idx) => (<ContactCard key={idx} {...item} />))}
                  </div>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12 max-w-5xl mx-auto">
              <a href="tel:1962" className="bg-[#1A1A2E] p-8 rounded-[45px] text-white flex items-center justify-between hover:bg-black transition-all group shadow-2xl border border-white/5">
                  <div className="flex items-center gap-8"><div className="w-20 h-20 bg-[#E74C3C] rounded-full flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform"><Phone size={36} fill="white"/></div><div className="text-left"><h4 className="text-5xl font-black italic tracking-tighter leading-none">1962</h4><p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2 leading-none">Animal Ambulance (Toll Free)</p></div></div>
                  <div className="bg-white/10 px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest">Dial</div>
              </a>
              <a href="tel:112" className="bg-[#1A1A2E] p-8 rounded-[45px] text-white flex items-center justify-between hover:bg-black transition-all group shadow-2xl border border-white/5">
                  <div className="flex items-center gap-8"><div className="w-20 h-20 bg-[#3498DB] rounded-full flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform"><Phone size={36} fill="white"/></div><div className="text-left"><h4 className="text-5xl font-black italic tracking-tighter leading-none">112</h4><p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2 leading-none">National Emergency Number</p></div></div>
                  <div className="bg-white/10 px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest">Dial</div>
              </a>
          </div>
      </section>
      ...
      {/* QUICK ACTIONS */}
      <div className="w-full max-w-4xl mx-auto px-4 space-y-4 mb-32">
          <button onClick={onOpenReport} className="w-full bg-[#C0392B] text-white p-6 md:p-8 rounded-[40px] flex items-center justify-between shadow-2xl hover:scale-[1.02] transition-all group active:scale-95">
              <div className="flex items-center gap-6"><div className="p-4 bg-white/10 rounded-2xl group-hover:rotate-12 transition-transform"><Camera size={28}/></div><div className="text-left"><h4 className="font-black italic uppercase text-lg md:text-xl tracking-tighter leading-none">Report Incident</h4><p className="text-[10px] font-bold opacity-60 uppercase mt-1 tracking-widest leading-none">Submit Live GPS Location</p></div></div>
              <div className="bg-white/20 px-6 py-2 rounded-2xl text-[11px] font-black uppercase group-hover:bg-white group-hover:text-[#C0392B] transition-all">Open</div>
          </button>
          <button onClick={onOpenVolunteer} className="w-full bg-[#16A085] text-white p-6 md:p-8 rounded-[40px] flex items-center justify-between shadow-2xl hover:scale-[1.02] transition-all group active:scale-95">
              <div className="flex items-center gap-6"><div className="p-4 bg-white/10 rounded-2xl group-hover:rotate-12 transition-transform"><Heart size={28} fill="white"/></div><div className="text-left"><h4 className="font-black italic uppercase text-lg md:text-xl tracking-tighter leading-none">Join Network</h4><p className="text-[10px] font-bold opacity-60 uppercase mt-1 tracking-widest leading-none">Register as a Volunteer</p></div></div>
              <div className="bg-white/20 px-6 py-2 rounded-2xl text-[11px] font-black uppercase group-hover:bg-white group-hover:text-[#16A085] transition-all">Join</div>
          </button>
      </div>
      ...
      <div className="fixed bottom-8 left-0 right-0 px-6 z-[1000] flex justify-center pointer-events-none">
        <button onClick={onSOS} className="pointer-events-auto w-full max-w-2xl bg-[#D35400] p-6 rounded-[35px] shadow-[0_20px_60px_rgba(211,84,0,0.5)] flex items-center justify-between text-white active:scale-95 transition-all group border-b-8 border-orange-950/20">
          <div className="flex items-center gap-4 text-left leading-none"><AlertCircle size={32} className="animate-pulse" /> <div><h3 className="text-2xl font-black italic uppercase tracking-tighter leading-none">Emergency SOS Help</h3><p className="text-[9px] font-bold uppercase opacity-70 tracking-widest mt-1 italic leading-none">Direct link to local responders</p></div></div>
...
const TeamMember = ({ name, role }: { name: string, role: string }) => (
    <div className="flex items-start gap-3 group px-1">
        ...
        <div><p className="text-[11px] font-black text-slate-800 uppercase tracking-tighter leading-none">{name}</p><p className="text-[9px] font-bold text-slate-400 mt-0.5 leading-tight">{role}</p></div>
    </div>
);

const NavBtn = ({ label, onClick }: any) => (<button onClick={onClick} className="text-[10px] font-black uppercase text-slate-500 hover:text-orange-600 transition-all tracking-widest border-b-2 border-transparent hover:border-orange-200 pb-1">{label}</button>);
...
const ContactCard = ({ district, name, phone }: any) => (
    <div className="bg-slate-50 p-5 rounded-[28px] border border-slate-100 flex items-center justify-between hover:bg-white hover:shadow-xl hover:border-orange-200 transition-all group active:scale-95 text-left">
        <div className="text-left"><p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none mb-1 text-left">{district}</p><h5 className="font-bold text-slate-800 text-[13px] leading-tight leading-none text-left">{name}</h5></div>
