...
import { Camera, MapPin, Send, X, AlertTriangle } from 'lucide-react';
...
const ReportForm = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newReport: AnimalReport = {
      ...
      type: formData.get('caseType') as string,
      details: formData.get('details') as string,
      location: "Srinagar (Auto-Detected GPS: 34.0837, 74.7973)",
      timestamp: new Date().toLocaleString('en-IN'),
      status: 'Pending'
    };

    // 2. Save to LocalStorage (This acts as our temporary Database)
    const existingReports = JSON.parse(localStorage.getItem('onepaw_reports') || '[]');
    const updatedReports = [newReport, ...existingReports];
    localStorage.setItem('onepaw_reports', JSON.stringify(updatedReports));

    // 3. Show Success to User after a small delay
      ...
      <div className="fixed inset-0 z-[10000] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-sm rounded-[40px] p-10 text-center shadow-2xl animate-in zoom-in duration-300">
          ...
          <p className="text-slate-500 mt-2 font-medium leading-relaxed text-sm">
            Your report is saved. Admin will verify and forward it to Srinagar rescue teams.
          </p>
    ...
    <div className="fixed inset-0 z-[10000] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-lg rounded-[40px] overflow-hidden shadow-2xl my-auto">
        <div className="bg-blue-600 p-8 text-white relative text-center">
          <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-white/20 rounded-full hover:bg-white/30"><X size={20}/></button>
          ...
          <p className="text-blue-100 text-[10px] mt-1 font-bold uppercase tracking-widest">Official Rescue Portal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-tighter">Your Location</p>
                <p className="text-xs font-bold text-slate-700 italic text-[10px]">Srinagar GPS Active</p>
              </div>
            </div>
          ...
          <div className="space-y-1">
            ...
            <textarea 
              name="details"
              placeholder="e.g. Near Batamaloo bridge, a brown dog is limping..."
          ...
          >
            {status === 'sending' ? "Logging Data..." : <><Send size={18}/> Submit to Admin</>}
          </button>
        </form>
      </div>
    </div>
